...
def main():
    ...
    #user_dict = {0:[ 18, 19, 61, 20, 6, 14, 22, 74, 4, 23],1:[67, 69, 33, 72, 49, 2, 55, 28, 76, 54, 24, 35, 78, 26, 42, 73, 32, 38, 64, 34, 70, 44, 57, 40, 30, 31, 41, 45, 29, 60, 56, 43, 58, 36, 39, 66, 63, 37, 27, 62, 51, 79, 65, 68, 47, 1, 12, 25, 52, 48]}
    #user_dict = {0:[  9, 75, 11, 17, 21, 18, 19, 61, 20, 6, 14, 22, 74, 4, 23],1:[67, 69, 33, 72, 49, 2, 55, 28, 76, 54, 24, 35, 78, 26, 42, 73, 32, 38, 64, 34, 70, 44, 57, 40, 30, 31, 41, 45, 29, 60, 56, 43, 58, 36, 39, 66, 63, 37, 27, 62, 51, 79, 65, 68, 47, 1, 12, 25, 52, 48]}
    ...
    # )
    train_loader, test_loader, _ = data.MSCOCO_text("ssl",args.data_dir, user_dict, batch_size=args.batch_size, normalize=args.normalize, size=args.size,F="N")
    infer_train_loader, infer_test_loader, _ = data.MSCOCO_text("base",args.data_dir, user_dict, batch_size=args.batch_size, normalize=args.normalize, size=args.size)
    ood_loader,_,_ = data.MSCOCO_text("base",args.data_dir, user_dict, batch_size=args.batch_size, normalize=args.normalize, size=args.size, F = "OOD")
    ...
    trainer = (trainers_text.ssl_with_text_CMA_CRA)
    ...
    for epoch in range(0, args.epochs):
        ...
        if epoch%1==0:
            ...
            features_train = get_features_text(
                textmodel, infer_train_loader
            )  # using feature befor MLP-head
            features_test = get_features_text(textmodel, infer_test_loader)
            ...
            features_ood = get_features_text(textmodel, ood_loader)
            ...
            # )
            print("AUROC=%f, AUPR=%f" %(auroc, aupr))

            AUROC_list.append(auroc)
            AUPR_list.append(aupr)
            print("MAX_AUROC:",max(AUROC_list))
            print("MAX_PR:",max(AUPR_list))
